...
@section('content')
    ...
    <div class="app-title">
        ...
        <ul class="app-breadcrumb breadcrumb">
            ...
            <li class="breadcrumb-item"><a href="#">{{ __('main.support') }}</a></li>
        </ul>
    </div>
    <div class="row justify-content-center">
        <div class="col-xs-12 col-md-12">
            <div class="tile">
                <h3 class="tile-title text-center">{{ __('main.support') }}</h3>
                <div class="homework_tl_div table-responsive">
                    <table class="table table-bordered">
                        ...
                        <tr role="row">
                            ...
                            <th scope="col" id="thumb" class="d-none d-md-table-cell text-center">
                                <span class="wc-image tips">{{ __('main.support_problem') }}</span>
                            </th>
                            <th scope="col" id="thumb" class="text-center">
                                ...
                                <td style="text-align: center;" class="">
                                    <div class="btn-group">
                                        @if(auth('admins')->user()->permissions->settings[5] == 1)
                                            <form action="{{ route('delete_support') }}" method="POST" class="delete_support_form">
                                                {{ csrf_field() }}
                                                <input type="hidden" name="support_id" value="{{ $message->id }}">
                                                <button class="btn btn-primary btn-sm delete_support_btn btn-border-red" type="submit"><i class="far fa-trash-alt no-margin" title="{{ __('main.delete_contact_us') }}"></i></button>
                                            </form>
                                        @endif
                                    </div>
                                </td>
