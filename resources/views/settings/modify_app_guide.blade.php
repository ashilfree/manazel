...
@section('content')
        ...
        <div class="row justify-content-center">
            <div class="col-xs-12 col-md-9">
                <div class="tile">
                    ...
                    <form action="{{ route('update_app_guide') }}" role="form" method="post" enctype="multipart/form-data" class="image_crop_form">
                        {{ csrf_field() }}
                        <input type="hidden" name="app_guide_id" value="{{ $app_guide->id }}">
                        <input type="hidden" id="filename" name="filename" /> <!-- Filename -->
                        <div class="form-group">
                            <label>{{ __('main.app_guide_title') }}</label>
                            <input class="form-control" type="text" name="title" value="{{ $app_guide->title }}" required autofocus>
                        </div>
                        <div class="form-group">
                            <label>{{ __('main.app_guide_en_title') }}</label>
                            <input class="form-control" type="text" name="en_title" value="{{ $app_guide->en_title }}" required autofocus>
                        </div>
                        <div class="form-group">
                            <label>{{ __('main.app_guide_description') }}</label>
                            <input class="form-control" type="text" name="description" value="{{ $app_guide->description }}" required autofocus>
                        </div>
                        <div class="form-group">
                            <label>{{ __('main.app_guide_en_description') }}</label>
                            <input class="form-control" type="text" name="en_description" value="{{ $app_guide->en_description }}" required autofocus>
                        </div>
                        <div class="form-group">
    ...
    @endif
    @if($app_guide->type == "youtube" || $app_guide->type == "video")
        <div class="row justify-content-center">
            <div class="col-xs-12 col-md-9">
                <div class="tile">
                    <h3 class="tile-title text-center">{{ __('main.edit_guide_video') }}</h3>
                    <form action="{{ route('update_app_guide_video') }}" role="form" method="post" enctype="multipart/form-data" class="image_crop_form">
                        {{ csrf_field() }}
                        <input type="hidden" name="app_guide_id" value="{{ $app_guide->id }}">
                        <div class="form-group">
                            <label>{{ __('main.app_guide_title') }}</label>
                        ...
                        <div class="form-group">
                            ...
                            @if($app_guide->type == "youtube")
                                <input type="text" name="videos_link" value="{{ $app_guide->image }}" class="form-control input_margin_bot mb-2 f-size-090">
