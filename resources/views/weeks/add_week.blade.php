...
@section('title',  __('main.add_week') )
...
@section('content')
    <div class="app-title">
        ...
        <ul class="app-breadcrumb breadcrumb">
            ...
            <li class="breadcrumb-item"><a href="#">{{ __('main.add_week') }}</a></li>
        </ul>
    </div>
    <div class="row justify-content-center">
        <div class="col-xs-12 col-md-9">
            <div class="tile">
                <h3 class="tile-title text-center">{{ __('main.add_week') }}</h3>
                <form action="{{ route('set_week') }}" role="form" method="post" enctype="multipart/form-data" class="image_crop_form game_upload">
                    {{ csrf_field() }}
                    <div class="form-group">
                        <label>{{ __('main.week_name') }}</label>
                        <input class="form-control" type="text" name="week_name" value="{{ old('week_name') }}" required autofocus>
                    </div>
                    <div class="form-group">
                        <label>{{ __('main.week_name_en') }}</label>
                        <input class="form-control" type="text" name="week_name_en" value="{{ old('week_name_en') }}" required>
                    </div>
                    <div class="form-group">
                    ...
                    <div class="form-group">
                        ...
                        <input class="form-control" type="text" name="tajweed_link" value="{{ old('tajweed_link') }}" required>
                    </div>
                    <div class="form-group">
                        <label>{{ __('main.tafsir_link') }}</label>
                        <input class="form-control" type="text" name="tafsir_link" value="{{ old('tafsir_link') }}" required>
                    </div>
                    <div class="form-group">
